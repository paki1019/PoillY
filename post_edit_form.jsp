<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" errorPage="PoillY_error.jsp" import="poilly.*,java.util.*,java.text.*"%>
...
<title>PoillY:<%=post_data.getMovie() %></title>
<!-- 합쳐지고 최소화된 최신 CSS -->
<link rel="stylesheet"
...
<body>
	<div class="container">
		...
		<div class="row" align="center">
			...
			%>
			<input type="hidden" name="movie" value='<%=post_data.getMovie()%>'>
			<input type="hidden" name="time" value="<%=date.format(today) +" "+ time.format(today) %>">
			<input type="hidden" name="view" value="<%=post_data.getView() %>">
			<input type="hidden" name="id" value="<%=post_data.getId()%>">
					<input type="hidden" name="writer"
						value="<%=account.getNickname()%>">
					<div class="form-group" align="left">
						<label>제목</label> <input type="text" name="title" maxlength="30"
							class="form-control" value="<%=post_data.getTitle()%>" placeholder="제목">
					</div>
					<div class="form-group" align="left">
						<label>별점</label><br> 
<span class="star-input">
	...
	<output for="star-input"><b>0</b>점</output>					
					...
					<div align="left">
					<label>스포일러</label><br><label class="radio-inline"> <input
						type="radio" name="spoiller" id="inlineRadio1" value="1">
						O
					</label> <label class="radio-inline"> <input type="radio"
						name="spoiller" id="inlineRadio2" value="0"> X
					</label> </div><div align="left"><label>내용</label></div>
					<textarea name="contents" class="form-control" rows="4"><%=post_data.getContents()%></textarea>
